...
def characterize_models():
    ...
    time_step = 0.002
    n_sims = 7
    no_of_points = [100000] * n_sims
    
    baro_start_s = [20, 20, 20, 1000, 1000, 20, 20]
    baro_stop_s = 1000 * np.ones(n_sims)
    
    pert_level = ['circulation',
                  'circulation', 'circulation',
                  'circulation', 'circulation',
                  'circulation', 'circulation']
    pert_variable = ['ventricle_wall_volume',
                     'ventricle_wall_volume', 'n_hs',
                     'ventricle_wall_volume', 'n_hs',
                     'ventricle_wall_volume', 'n_hs']
    pert_start_s = 40 * np.ones(n_sims)
    pert_stop_s = 40.1 * np.ones(n_sims)
    pert_total_change = [0.0, 0.03, 15000, 0.03, 15000, 0.03, 15000]
    
    growth_start_s = [1000, 1000, 1000, 1000, 1000, 60, 60]
    growth_stop_s = 1000 * np.ones(n_sims)
    ...
    for i in range(len(no_of_points)):
        ...
        p['growth'] = dict()
        p['growth']['activations'] = []
        gr = dict()
        gr['t_start_s'] = growth_start_s[i]
        gr['t_stop_s'] = growth_stop_s[i]
        p['growth']['activations'].append(gr)
